...
def test_groupby_dayofyear():
    ...
    for op in 'min', 'max', 'mean', 'sum':
        xarray.testing.assert_equal(getattr(blocked_doy, op)(), getattr(xarray_doy, op)())

def test_groupby_dayofyear_dask():
    time = pandas.date_range('20020101','20050101', freq='D', closed='left')
    hourly = xarray.DataArray(dask.array.zeros(time.size, chunks=50), coords=[('time', time)])
...
def test_groupby_monthday():
    ...
    for op in 'min', 'max', 'mean', 'sum':
        numpy.testing.assert_array_equal(getattr(blocked_doy, op)(), getattr(xarray_doy, op)())
...
def test_groupby_climatology():
    time = pandas.date_range('20020101','20050101', freq='D', closed='left')
    hourly = xarray.DataArray(dask.array.random.random(time.size, chunks=50), coords=[('time', time)])
...
def test_groupby_percentile():
    ...
    climatology_xr = hourly.load().groupby('time.dayofyear').reduce(numpy.percentile, q=90)
